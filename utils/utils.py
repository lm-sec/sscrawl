...
class Utils:
    ...
    def is_username_like(field_name: str) -> bool:
        ...
        return re.match("^\s*((user[ \_\-]?((name)|(id))?)|(((role)|(client))[ \_\-]?id))\s*$", field_name, re.IGNORECASE) is not None
    ...
    def is_password_like(field_name: str) -> bool:
        ...
        return re.match("^\s*((pass[ \_\-]?(word)?)|(secret[ \_\-]?(id)?))\s*$", field_name, re.IGNORECASE) is not None
