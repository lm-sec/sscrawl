...
def get_secrets(domain: str, username: str, password: str, is_hash: bool, out_folder: str, out_file: str, node: SSNode, found_history: set, recursive: bool, thread_count: int, secret_server: SecretServer):
    ...
    for auth_method in secret_server.authentication_methods:
        ...
        try:
            ...
            auth_success = secret_server.connect_session(domain, username, password, session, auth_method, is_hash)
        ...
        for i in range(thread_count):
            if len(secrets_list[i]) > 0:
                ...
                get_secrets_threads.append(GetSecretsThread(secrets_list[i], found_children[len(found_children) - 1], session, secret_lines_output[len(secret_lines_output) - 1], secret_server, found_history, auth_method))
    ...
    if logger.verbose:
        for child in node.children:
            ...
            logger.console_logger.debug(f"Found secret ID {child.readable_id}{child_data if len(child_data) > len(child_data_separator) else ''}")
...
def recursive_generate_graph(node: SSNode, file: TextIOWrapper, parentId: str, arrow_strings: set, print_already_found: bool, print_access_denied: bool):
    if (not node.already_found and not node.got_denied) or (print_already_found and node.already_found) or (print_access_denied and node.got_denied):
        file.write(node_to_str(node, parentId))
    for n in node.children:
        if (not node.already_found and not n.already_found and not node.got_denied and not n.got_denied) or (print_already_found and n.already_found) or (print_access_denied and n.got_denied):
...
parser = argparse.ArgumentParser(description="SSCrawl crawls secret servers for secrets and files.", formatter_class=SortingHelpFormatter)

parser.add_argument("-d", "--domain", help="The active directory domain name to be prepended to the username during connection as domain\\username", default="", required=False)
parser.add_argument("-s", "--server", help=f"The secret server type/vendor [{', '.join(SUPPORTED_SERVERS)}]", required=True, type=str, choices=SUPPORTED_SERVERS)
parser.add_argument("-u", "--user", help="The username to authenticate to the secret server. Can be a role_id.", required=True, type=str)
parser.add_argument("-p", "--pwd", help="The password to authenticate to the secret server Can be a secret_id.", required=False, type=str)
parser.add_argument("-l", "--url", help=f"The URL of the secret server API", required=True, type=str)
parser.add_argument("-o", "--out", help=f"The output secret file name, default {DEFAULT_SSCRAWL_OUT}", required=False, type=str, default=DEFAULT_SSCRAWL_OUT)
...
logger = SSCrawlLogger(l, args.verbose, out_file, args.outfolder)
