...
from secret_servers.hashicorp_vault.hashicorp_vault_constants import API_V1, AUTHENTICATION_APPROLE_PATH, AUTHENTICATION_METHOD_APPROLE, AUTHENTICATION_METHOD_USER_PASS, AUTHENTICATION_USERPASS_PATH, IDENTITY_ROUTE, LISTING_SECRETS_ENGINES_PATH, SECRET_ENGINE_CUBBYHOLE, SECRET_ENGINE_KV, SECRET_ENGINE_KVV1, SECRET_ENGINE_KVV2, SECRETS_ENGINES_FIELD, SYS_ROUTE, VAULT_TOKEN_HEADER
...
class HashicorpVaultSecretServer(SecretServer):
    ...
    def connect_session(self, domain: str, username: str, password: str, session: requests.Session, authentication_method: str, is_hash: bool) -> bool:
        ...
        if is_hash:
            raise AuthenticationMethodNotSupportedException("Hashicorp vault login does not support pass-the-hash. A hash cannot be supplied to login.")
        ...
        if authentication_method == AUTHENTICATION_METHOD_USER_PASS:
            ...
            if not Utils.is_valid_uuid(username):
                self.logger.console_logger.debug("The client_id is not in the form of a UUID. It is unexpected for the approle login method. Skipping.")
                return False
    ...
    def list_secrets(self, session: requests.Session, authentication_method: str) -> 'list[HashicorpSecretListItem]':
        ...
        for key in engine_routes:
            ...
            if engines_dict[key]["type"] == SECRET_ENGINE_KV:
                if "options" in engines_dict[key] and "version" in engines_dict[key]["options"] and engines_dict[key]["options"]["version"] == "2":
    ...
    def _list_secrets_recursive(self, session: requests.Session, engine_path: str, engine_type: str, current_local_path: str, secrets_list: 'list[HashicorpSecretListItem]'):
        res = {}
        ...
        if res.status_code == 200:
            ...
            for secret in new_secrets_list:
                if secret[-1] == "/":
                    self._list_secrets_recursive(session, engine_path, engine_type, current_local_path + secret, secrets_list)
                else:
                    secrets_list.append(HashicorpSecretListItem(engine_type, engine_path, current_local_path + secret))
    
    def get_secrets_threaded(self, secret_items: 'list[HashicorpSecretListItem]', found_secrets_list: 'list[SSNode]', session: requests.Session, found_ids_history: set, secret_lines_output: 'list[str]', authentication_method: str):
        ...
        for secret in secret_items:            
            ...
            data_dict = response_json["data"] if secret.secret_engine_type == SECRET_ENGINE_CUBBYHOLE or secret.secret_engine_type == SECRET_ENGINE_KVV1 else response_json["data"]["data"]
            keys = data_dict.keys()
            ...
            for key in keys:
                ...
                try:
                    if Utils.is_file_like(data_dict[key]):
                        ...
                        secret_lines_output.append(self.logger.get_secret_log_line([secret.unique_id, secret.secret_local_path, key, file_name]))
                    else:
                        secret_lines_output.append(self.logger.get_secret_log_line([secret.unique_id, secret.secret_local_path, key, data_dict[key]]))
                except:
                    self.logger.console_logger.debug(f"Error while handling the secret at {secret.unique_id}, will attempt to continue")
    ...
    def _extract_client_token(self, session: requests.Session, response: requests.Response, auth_method: str) -> bool:
        ...
        if not content_json["auth"] or not content_json["auth"]["client_token"]:
            self.logger.console_logger.debug(f"No auth.client_token in response content, {auth_method} authentication failed")
            return False
